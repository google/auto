...
class BuilderMethodClassifier {
  ...
  private boolean classifyMethods(TypeElement builderType, boolean autoValueHasToBuilder) {
    Set<ExecutableElement> methods = getLocalAndInheritedMethods(
        builderType, typeUtils, elementUtils);
    Set<ExecutableElement> abstractMethods = Sets.filter(methods, MoreElements.hasModifiers(Modifier.ABSTRACT));
