...
public class AutoValueJava8Test {
  private static boolean javacBug;

  // This is appalling. Some versions of javac do not correctly report annotations on type uses.
  // Since some of the tests here are for exactly that, we compile a test program with a test
  // annotation processor to see whether we might be in the presence of such a javac, and if so
  // we skip the tests that would fail because of the bug. This isn't completely sound because we
  // can't be entirely sure that the javac that Compiler.javac() finds is the same as the javac
  // that was used to build this test (and therefore run AutoValueProcessor), but it's better than
  // just ignoring the tests outright.
  @BeforeClass
  public static void setUpClass() {
    ...
    if (!compilation.errors().isEmpty()) {
      javacBug = true;
      System.err.println("THIS SYSTEM'S JAVAC DOES NOT HANDLE ANNOTATED CORRECTLY");
    }
  }
